...
--[[ Local Functions ]]
local function sendSubscribeButtonRPC(pAppId1, pAppId2, pButtonName, pBothSubscribed)
  local mobSession1 = common.mobile.getSession(pAppId1)
  local mobSession2 = common.mobile.getSession(pAppId2)
  local cid = mobSession1:SendRPC("SubscribeButton", {buttonName = pButtonName})
  local pTime = 0
  if pBothSubscribed then pTime = 1 end
    common.hmi.getConnection():ExpectNotification("Buttons.OnButtonSubscription",
        {name = pButtonName, isSubscribed = true, appID = common.app.getHMIId(pAppId1) })
    :Do(function()
      ...
      mobSession1:ExpectNotification("OnButtonEvent",{buttonName=pButtonName,buttonEventMode="BUTTONDOWN"})
      mobSession2:ExpectNotification("OnButtonEvent",{buttonName=pButtonName,buttonEventMode="BUTTONDOWN"}):Times(pTime)
      ...
      mobSession1:ExpectNotification("OnButtonPress",{buttonName = pButtonName, buttonPressMode = "LONG"})
      mobSession2:ExpectNotification("OnButtonPress",{buttonName = pButtonName, buttonPressMode = "LONG"}):Times(pTime)
    end)
    mobSession1:ExpectResponse(cid, { success = true, resultCode = "SUCCESS" })
    mobSession1:ExpectNotification("OnHashChange")
end
...
runner.Step("App1 from Mobile 1 requests Subscribe on OK", sendSubscribeButtonRPC, {1, 2, "OK"})

runner.Step("Activate App 2", common.app.activate, { 2 })
runner.Step("App2 from Mobile 2 requests Subscribe on button_1", sendSubscribeButtonRPC, {2, 1, "PLAY_PAUSE" })
