...
local function sendOnVehicleData(pAppId1, pAppId2, pNumberOfAppToBeNotified)
  local mobSession1 = common.mobile.getSession(pAppId1)
  local mobSession2 = common.mobile.getSession(pAppId2)
  local pTime1, pTime2
  local pTime = pNumberOfAppToBeNotified                    -- defines which app should get this notification

  if     pTime == 0 then pTime1 = 0; pTime2 = 0
  elseif pTime == 1 then pTime1 = 1; pTime2 = 0
  elseif pTime == 2 then pTime1 = 0; pTime2 = 1
  end
...
runner.Step("Activate App 1", common.app.activate, { 1 })                                   -- just for check
runner.Step("HMI sends OnVehicleData - Apps 1 receives", sendOnVehicleData, { 1, 2, 1 })    -- BUG here
