...
-- test.mobileConnection = {} -- It will be initialized by prepareMobileConnectionTable function
test.mobileSession = {}
...
local function prepareMobileConnectionTable()
  if test.mobileConnection then
    if test.mobileConnection.connection then
      ...
      table.insert(test.mobileConnection, defaultMobileConnection)
    end
  else
    test.mobileConnection = {}
  end
...
--! @parameters: none
--! @return: Allow SDL event expectation
--]]
...
--[[ Functions of network submodule ]]

function m.network.addInterface(pId, pAddress)
  os.execute("ifconfig lo:" .. pId .." " .. pAddress)
end

function m.network.deleteInterface(pId)
...
function m.mobile.createConnection(pMobConnId, pMobConnHost, pMobConnPort)
  ...
  test.mobileConnection[pMobConnId] = connection
...
function m.mobile.deleteConnection(pMobConnId)
  ...
  connection:Close()
  test.mobileConnection[pMobConnId] = nil
end

function m.mobile.getConnection(pMobConnId)
  if pMobConnId == nil then pMobConnId = 1 end
  return test.mobileConnection[pMobConnId]
end
...
function m.mobile.createSession(pAppId, pMobConnId, pMobSesionConfig)
  ...
  local session = mobileSession.MobileSession(test, test.mobileConnection[pMobConnId])
...
function m.mobile.getApps(pMobConnId)
  ...
  for _, mobileSession in ipairs(test.mobileSession) do
    if mobileSession.connection == test.mobileConnection[pMobConnId] then
...
function m.app.activate(pAppId)
  if not pAppId then pAppId = 1 end
  local requestId = m.hmi.getConnection():SendRequest("SDL.ActivateApp", { appID = m.getHMIAppId(pAppId) })
  m.hmi.getConnection():ExpectResponse(requestId)
...
--[[ Functions of ATF connections extension ]]
